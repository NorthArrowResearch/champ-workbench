...
The primary purpose of the CHaMP Workbench is to help users work with the existing, pre-defined visits associated with the CHaMP program. The official set of visits is defined on the central [CHaMP Monitoring](http://champmonitoring.org) database and then [synchronized](/technical_reference/data_menu/synchronize_champ_data) down the the local Workbench database.
...
![Custom Visits](/images/custom_visits.png)
