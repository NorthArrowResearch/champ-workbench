...
namespace CHaMPWorkbench.Data
{
    public partial class frmScavengeVisitInfo : Form
    {
        ...
        {
            String sSQL = "SELECT SiteID AS ID, SiteName AS Title, WatershedID FROM VisitInformation WHERE (SiteID IS NOT NULL) AND (SiteName IS NOT NULL) GROUP BY SiteID, SiteName, WatershedID";
            using (OleDbCommand dbCom = new OleDbCommand(sSQL, dbCHaMP))
            {
                OleDbDataReader dbRead = dbCom.ExecuteReader();
                while (dbRead.Read())
        ...
        {
            String sSQL = "SELECT VisitID AS ID, HitchName, CrewName, VisitDate, SiteID, [Primary Visit], HitchID FROM VisitInformation WHERE (VisitID IS NOT NULL) AND (SiteID IS NOT NULL)";
            using (OleDbCommand dbCom = new OleDbCommand(sSQL, dbCHaMP))
            {
                OleDbDataReader dbRead = dbCom.ExecuteReader();
                while (dbRead.Read())
        ...
        private void UpdateSegmentsAndUnits(OleDbConnection dbCHaMP, RBTWorkbenchDataSetTableAdapters.CHaMP_SegmentsTableAdapter daSegments, RBTWorkbenchDataSetTableAdapters.CHAMP_ChannelUnitsTableAdapter daUnits, RBTWorkbenchDataSet ds)
        {
            ...
            sSQL = "INSERT INTO CHAMP_Segments (VisitID, SegmentNumber, SegmentName) SELECT VisitID, 1, 'Main Channel' FROM CHaMP_Visits WHERE VisitID NOT IN (SELECT VisitID FROM CHaMP_Segments GROUP BY VisitID)";
            ...
            sSQL = "SELECT VisitID, ChannelSegment, Tier1, Tier2, ChannelUnitNumber FROM ChannelUnit WHERE (VisitID IS NOT NULL) AND (ChannelSegment IS NOT NULL)";
            dbCom = new OleDbCommand(sSQL, dbCHaMP);
